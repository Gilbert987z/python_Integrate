...
'''    deviceTypeId = jsonData.get('datas')[0].get('deviceTypeId')  # 获取得到deviceTypeId
    ...
    response_vin = jsonData.get('datas')[0].get('vin')  # 获取得到vin'''

'''
# 升级任务设置-获取升级任务设置的信息
def upgrade_task_setting_msg(cookies, deviceTypeId):
    """
    接口：升级任务设置，获取升级任务设置的信息
    """
    print('------升级任务设置--------')
    url = chery_url + '/api/getHardwareByDevice'
    params = {
...
# 版本
#
url = '/api/queryAvailableVersion?modelId=97&deviceTypeId=160'
'''
...
def task_excel_input(account):
    while True:
        try:
            path = input('请输入Excel的路径(I:/data.xls)：')
            sheet = input('请输入Excel的工作表名（Sheet1）：')

            # path = 'I:/data.xls'
            # sheet = 'Sheet1'

            task_list = get_excel_data(path, sheet)
            print('\nTBOX升级任务：', task_list)
            print('共有 %d 个TBOX升级任务' % len(task_list))
            # time.sleep(3)   #暂停三秒
            break
        except BaseException:
            print('\nERROR:输入错误，请重新输入！')
    ...
    for task_num in range(0, len(task_list)):
        ...
        tbox_process(VIN, taskName, account, lastVersion)  # 登录后的总流程
...
def task_console_input(account):
    ...
    tbox_process(VIN, taskName, account, inLastVersion='')  # 登录后的总流程
...
if __name__ == '__main__':
   ...
   # sys.stdout = Logger('D:\\a.log', sys.stdout)
    ...
    while flag:
        ...
        print('\n（exit-结束运行,并关闭当前窗口，1-重新运行程序,2-打印输出运行数据）')
        while True:
            try:
                ...
                if flag == 'exit':
                    ...
                    print('\n<<<<打印输出运行数据>>>>')
                    sys.stdout = Logger("D:\\1.txt", sys.stdout)  # 这里我将Log输出到D盘
