...
def task_excel_input(account):
    while True:
        try:
            file_path = input('请输入Excel的路径(I:\data.xlsx)：')
    ...
    for task_num in range(0, task_sum):
        ...
        task_result_status = task_result_datas.get('otaList')[0].get('status')
        print('\n第%d次升级结果：' % task_count, task_result_status)

        write_excel_xls(file_path, task_count, excel_data[1], task_result_status)  # 写入升级结果的数据

        if task_result_status == 'UPGRADES_SUCCESS':
            task_result_success = task_result_success + 1

    task_success_rate = task_result_success / task_sum * 100
